...
for dataset_type in folder_names:
    ...
    image_folder = os.path.join(dataset_path, dataset_type, 'mask')
    ...
    for image_name in image_names:
        ...
        mask = Image.open(os.path.join(image_folder, image_name))
        mask = np.array(mask)
        ...
        mask_pre_process = (mask == 255) * mask
        # resized_mask = resized_mask.astype('uint8')
        mask_pre_process = mask_pre_process.astype('uint8')
        ...
        imageio.imwrite(target_path, mask_pre_process)
