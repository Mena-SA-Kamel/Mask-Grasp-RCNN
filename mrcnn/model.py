...
def generate_zero_targets(grasping_anchors, config):
    grasp_anchor_match = tf.ones([0, config.GRASP_ANCHORS_PER_ROI, 1]) * -1
...
def generate_grasping_targets(grasp_overlaps, grasping_anchors, final_roi_gt_grasp_boxes, config):
    ...
    grasp_roi_iou_max = tf.reduce_max(grasp_overlaps, axis=-1)

    # Specifies the max overlap anchor for each GT box: shape [num of ROIs, num of GT grasp boxes, 1]
    # Unmatched boxes match to anchor #0

    # non_zero_grasp_boxes: specifies the GT grasp boxes which are non zeros (not zeros padded instances)
    non_zero_grasp_boxes = tf.cast(tf.reduce_sum(tf.abs(grasp_overlaps), axis=-1), tf.bool)
    non_zero_grasp_box_indices = tf.where(tf.cast(tf.reduce_sum(tf.abs(grasp_overlaps), axis=-1), tf.bool))

    grasp_roi_iou_argmax = tf.expand_dims(tf.argmax(grasp_overlaps, axis=-1), axis=-1)
    ...
    grasp_box_ix = tf.tile(tf.range(tf.shape(grasp_overlaps)[1]), [tf.shape(grasp_overlaps)[0]])
    grasp_box_ix = tf.cast(tf.reshape(grasp_box_ix, tf.shape(grasp_roi_iou_argmax)), dtype=tf.int64)
    ...
    anchors_to_keep = tf.where(grasp_overlaps > config.ARIOU_POS_THRESHOLD)
    ...
    # grasp_box_match = tf.Print(grasp_box_match,
    #                               [tf.shape(grasp_box_match)],
    #                               message='grasp_box_match',
    #                               summarize=-1)
    # positive_grasp_box_locations = tf.Print(positive_grasp_box_locations,
    #                            [tf.shape(positive_grasp_box_locations)],
    #                            message='positive_grasp_box_locations',
    #                            summarize=-1)
...
def grasping_overlaps_graph(anchors, boxes):
    ...
    # Anchors are normalized while boxes are not!

    grasping_anchors_reshaped = tf.tile(anchors, [1, tf.shape(boxes)[1], 1])
    grasping_boxes_reshaped = tf.tile(K.expand_dims(boxes, axis=2),
                                      [1, 1, tf.shape(anchors)[1], 1])
    grasping_boxes_reshaped = tf.reshape(grasping_boxes_reshaped, tf.shape(grasping_anchors_reshaped))

    grasp_anchors = tf.reshape(grasping_anchors_reshaped, [-1, 5])
    gt_grasp_boxes = tf.reshape(grasping_boxes_reshaped, [-1, 5])
...
def process_gt_grasp_boxes(positive_rois, roi_gt_boxes, roi_gt_grasp_boxes, roi_gt_grasp_class, config):
    '''
    This takes the positive proposals, defined in positive_rois, and the corresponding GT rois, defined by roi_gt_boxes,
    ...
    # Calculates dx and dy defined as the difference in [x1, y1] of positive_rois, roi_gt_boxes [y1, x1, y2, x2]

    # proposal_center_x = (proposal_x1 + proposal_x2) / 2.0
    # proposal_center_y = (proposal_y1 + proposal_y2) / 2.0
    #
...
def detection_targets_graph(proposals, gt_class_ids, gt_boxes, gt_masks, anchors, gt_grasp_class, gt_grasp_boxes, config):
    ...
    num_positives = tf.reduce_sum(tf.cast(positive_roi_bool, dtype=tf.float32))
    positive_indices = tf.cond(
        tf.greater(num_positives, 0.),
...
class DetectionTargetLayer(KE.Layer):
    ...
    def compute_output_shape(self, input_shape):
        return [
            ...
            (None, None, self.config.GRASP_ANCHORS_PER_ROI, 5), # grasping anchors
            (None, None, self.config.GRASP_ANCHORS_PER_ROI, 1), # grasping box class
            (None, None, self.config.GRASP_ANCHORS_PER_ROI, 5), # grasping box deltas
        ]
...
#  Detection Layer
############################################################
def refine_mask_grasp_detections_graph(rois, probs, deltas, window, grasp_probs, grasp_bbox, config):
    ...
    roi_duplicates = tf.gather(refined_rois, keep)
    roi_duplicates = tf.tile(tf.expand_dims(roi_duplicates, axis=1), [1, config.GRASP_ANCHORS_PER_ROI, 1])

    class_ids_duplicates = tf.cast(tf.gather(class_ids, keep), tf.float32)[..., tf.newaxis]
    class_ids_duplicates = tf.tile(tf.expand_dims(class_ids_duplicates, axis=1), [1, config.GRASP_ANCHORS_PER_ROI, 1])

    class_scores_duplicates = tf.gather(class_scores, keep)[..., tf.newaxis]
    class_scores_duplicates = tf.tile(tf.expand_dims(class_scores_duplicates, axis=1), [1, config.GRASP_ANCHORS_PER_ROI, 1])

    # Arrange output as [N, (y1, x1, y2, x2, class_id, score)]
    # Coordinates are normalized.
    # detections: rois:0-3, class_ids: 4, scores: 5, grasp_boxes: 6:10, grasp_probs: 11-12
    detections = tf.concat([
        roi_duplicates,
        # tf.to_float(tf.gather(class_ids, keep))[..., tf.newaxis],
        class_ids_duplicates,
        class_scores_duplicates,
        tf.gather(grasp_bbox, keep),
        tf.gather(grasp_probs, keep),
        ], axis=2)

    # Pad with zeros if detections < DETECTION_MAX_INSTANCES
    gap = config.DETECTION_MAX_INSTANCES - tf.shape(detections)[0]
    detections = tf.pad(detections, [(0, gap), (0, 0), (0, 0)], "CONSTANT")
    return detections
...
class GraspDetectionLayer(KE.Layer):
    ...
    def __init__(self, config=None, **kwargs):
        super(GraspDetectionLayer, self).__init__(**kwargs)
        self.config = config
    ...
    def call(self, inputs):
        ...
        detections_batch = utils.batch_slice(
            [rois, mrcnn_class, mrcnn_bbox, window, grasp_probs, grasp_bbox],
            lambda u, v, x, y, w, z: refine_mask_grasp_detections_graph(u, v, x, y, w, z, self.config),
            self.config.IMAGES_PER_GPU)
        ...
        return tf.reshape(
            detections_batch,
            [self.config.BATCH_SIZE, self.config.DETECTION_MAX_INSTANCES, self.config.GRASP_ANCHORS_PER_ROI, 13])

    def compute_output_shape(self, input_shape):
        return (None, self.config.DETECTION_MAX_INSTANCES, self.config.GRASP_ANCHORS_PER_ROI, 13)
...
def grasp_loss_graph(config, target_bbox, target_class, bbox, class_logits, roi_class_ids):
    ...
    for i in range(config.BATCH_SIZE):
        ...
        positive_anchor_mask = K.cast(K.equal(batch_target_class_filtered, 1), tf.int32)

        negative_anchor_mask = K.cast(K.equal(batch_target_class_filtered, -1), tf.int32)
        ...
        top_negative_losses_sum = tf.cond(
            ...
            false_fn=lambda: tf.cast(tf.zeros([0, config.GRASP_ANCHORS_PER_ROI, 1]), tf.float32)
        )
...
def load_image_gt(dataset, config, image_id, augment=False, augmentation=None, online_augment = False, pre_augment = False,
    ...
    mode: If set to 'grasping_points', loads the ground truth bounding boxes and
        ommits the masks
    ...
    if mode == 'grasping_points':
        bbox = bbox_resize_5_dimensional
    else:
...
def build_grasping_targets(anchors, gt_class_ids, gt_boxes, config):
    ...
    top_match_anchors = np.argmax(overlaps, axis=0)
    top_iou_values = np.max(overlaps[top_match_anchors, :], axis=1)

    # gt_iou_argmax = np.argwhere(overlaps == np.max(overlaps, axis=0))[:, 0]
    gt_iou_argmax = np.argwhere(overlaps == top_iou_values)[:, 0]
...
def build_rpn_targets(anchors, gt_class_ids, gt_boxes, config, mode=''):
    ...
    if mode == 'grasping_points':
        ...
        if total_num_negative == 0:
            neutral_ids = np.where(rpn_match == 0)[0]
            positive_ids = np.where(rpn_match == 1)[0]
            negative_ids = np.random.choice(neutral_ids, config.RPN_TRAIN_ANCHORS_PER_IMAGE - len(positive_ids), replace=False)
            rpn_match[negative_ids] = -1
...
class MaskRCNN():
    ...
    def build_mask_grasp_rcnn_model(self, mode, config):
        ...
        if mode == "training":
            ...
            rois, target_class_ids, target_bbox, target_mask, grasping_anchors, target_grasp_anchor_match, target_grasp_deltas = \
                DetectionTargetLayer(config, name="proposal_targets")([
            ...
            grasp_loss = KL.Lambda(lambda x: grasp_loss_graph(config, *x), name="grasp_loss")(
                [target_grasp_deltas, target_grasp_anchor_match, grasp_bbox, grasp_class_logits, target_class_ids])
    ...
    def compile(self, learning_rate, momentum, task=''):
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers,
              augmentation=None, custom_callbacks=None, no_augmentation_sources=None, task=''):
        """Train the model.
        ...
        self.compile(learning_rate, self.config.LEARNING_MOMENTUM, task=task)
    ...
    def unmold_grasping_detections(self, rpn_rois, rpn_class, rpn_bbox, original_image_shape,
                          image_shape, window):
        """Reformats the detections of one image from the format of the neural
        network output to a format suitable for use in the rest of the
        application.

        rpn_rois: [6000, (x, y, w, h, theta)] in normalized coordinates
        rpn_class: [N, 2] BG/FG
        rpn_bbox: [N, (dy, dx, log(dh), log(dw), dtheta)]
        ...
        Returns:
        rpn_rois: [6000, (x, y, w, h, theta)] Bounding boxes in pixels
        rpn_class: [N, 2] BG/FG
        rpn_bbox: [N, (dy, dx, log(dh), log(dw), dtheta)]
        """
        # Convert boxes to pixel coordinates on the original image
        boxes = utils.denorm_boxes(rpn_rois, original_image_shape[:2], mode='grasping_points')
        boxes[:, -1] = boxes[:, -1] / (2 * 360)
    ...
    def unmold_mask_grasp_detections(self, detections, mrcnn_mask, original_image_shape,
        ...
        return boxes, class_ids, scores, full_masks, grasp_boxes, grasp_probs
...
def denorm_boxes_graph(boxes, shape):
    ...
    """
    h, w = tf.split(tf.cast(shape, tf.float32), 2)
    scale = tf.concat([h, w, h, w], axis=-1) - tf.constant(1.0, dtype=tf.float32)
    shift = tf.constant([0., 0., 1., 1.])
